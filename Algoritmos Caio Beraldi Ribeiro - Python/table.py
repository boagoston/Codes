...
class Table:
    ...
    def linearRecursionSearch(self, index, value):
        if index >= self.size:
            return -1

        elif self.value[index] == value:       
            print("Achou por busca linear recursiva com key:", self.key[index], "com valor de:", self.value[index])
            return index + 1

        return self.linearRecursionSearch(index + 1, value)
            ...
            if(key == self.key[self.size]):
                ...
                if(begin <= end):
                    ...
                    if (self.key[half] == key):
                        ...
                        print("Achou por busca binária recursiva com key:", self.auxI, "com valor de:", self.value[half])
                        return self.auxI
